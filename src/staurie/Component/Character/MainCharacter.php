...
namespace App\Staurie\Component\Character;

use App\Staurie\Component\AbstractComponent;
use App\Staurie\Component\Character\CoreFunctions\EquipFunction;
use App\Staurie\Component\Character\CoreFunctions\MainCharacterFunction;
use App\Staurie\Component\Character\CoreFunctions\SpeakFunction;
use App\Staurie\Component\Character\CoreFunctions\StatsFunction;
use App\Staurie\Component\Character\CoreFunctions\UnequipFunction;
...
use App\Staurie\Game\Item_Equippable;
...
class MainCharacter extends AbstractComponent {
    ...
    final protected function new() {
        ...
        if($this->config['ask_gender']) {
            $valid = false;
            while(!$valid) {
                $input = strtolower(trim(readline('Character gender (male/female) : ')));
                if(in_array($input, ['male','female','m','f'], true)) {
                    $this->gender = ($input === 'm') ? 'male' : (($input === 'f') ? 'female' : $input);
                    $valid = true;
                } else {
                    $pp = $this->container->getPrettyPrinter();
                    $pp?->writeLn('Please choose "male" or "female"', 'red');
                }
            }
        }
