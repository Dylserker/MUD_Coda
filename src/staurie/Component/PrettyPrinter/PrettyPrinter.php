...
namespace App\Staurie\Component\PrettyPrinter;
...
class PrettyPrinter extends AbstractComponent {
    ...
    private function getTerminalColumns(): int {
        if (PHP_OS_FAMILY === 'Windows') {
            return 100;
        }
        $columns = @exec('tput cols 2> /dev/null');
        if(!is_numeric($columns) || (int)$columns <= 0) { return 100; }
        return (int)$columns;
    }
    ...
    {
        if($centered) {
            $columns = $this->getTerminalColumns();
    ...
    {
        if(count($lines) == 0) {
            // Ne rien afficher si le tableau est vide
            return;
        }
